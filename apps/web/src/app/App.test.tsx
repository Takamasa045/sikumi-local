...
describe('Shikumi Local garden', () => {
  it('opens the garden as the home screen', async () => {
    ...
    expect(
      screen.getByRole('link', { name: 'Shikumi Local ホーム' }),
    ...
    expect(screen.queryByRole('form', { name: /仕事/ })).not.toBeInTheDocument()
    await waitFor(() => {
  ...
  it("shows today's workshop without pretending work is running", async () => {
    ...
    expect(screen.getByRole('heading', { name: '登録した場所' })).toBeVisible()
  ...
  it('fills a path from the native folder picker before registering', async () => {
    fetchMock.mockImplementation(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          url.endsWith('/api/workspaces/choose-folder') &&
          method === 'POST'
        ) {
          return jsonResponse({
            cancelled: false,
    ...
    await openTodayWorkshop()
    await userEvent.type(
      screen.getByLabelText('担当の名前（任意）'),
      'ブログ番',
    )
    await userEvent.click(
  ...
  it('removes a registered place from today, the garden, and settings', async () => {
    ...
    await userEvent.click(
      screen.getByRole('button', { name: 'この場所を外す' }),
    )
    await waitFor(() => {
  ...
  it('lists registered repository activity without claiming an AI source', async () => {
    ...
    expect(screen.getByText('ログインまわりに、途中の仕事がある')).toBeVisible()
    expect(screen.queryByText(/しまっていない変更/)).toBeNull()
  ...
  it('restores a persisted workspace on load', async () => {
    ...
    expect(screen.getByText('ログインまわりに、途中の仕事がある')).toBeVisible()
    expect(screen.queryByText(/しまっていない変更/)).toBeNull()
    expect(screen.getByRole('button', { name: 'この場所を追加' })).toBeVisible()
  })
  ...
  it('keeps an add-repository path after the first workspace exists', async () => {
    ...
    expect(screen.getByText('my-project番')).toBeVisible()
    expect(screen.getByText('second-project番')).toBeVisible()
    expect(screen.getByTestId('workspace-line')).toHaveTextContent('2 件の場所')
  ...
  it('shows a registration error from the local server', async () => {
    ...
    await openTodayWorkshop()
    await userEvent.type(screen.getByLabelText('場所のパス'), '/tmp/not-git')
    await userEvent.click(
      screen.getByRole('button', { name: 'この場所を追加' }),
    )
