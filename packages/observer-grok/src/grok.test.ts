...
describe('grok event mapping', () => {
  it('canonicalizes aliases and classifies unknown future names', () => {
    expect(isGrokHookEvent('SessionStart')).toBe(true)
    expect(isGrokHookEvent('Nope')).toBe(false)
    expect(canonicalizeGrokEventName('')).toBe('unknown')
    expect(canonicalizeGrokEventName('session_start')).toBe('SessionStart')
    expect(matcherForGrokEvent('PreToolUse')).toBe('*')
    expect(matcherForGrokEvent('SessionStart')).toBeUndefined()
    expect(mapGrokEvent('user_prompt_submit').normalizedType).toBe(
      'prompt.submitted',
    )
    expect(mapGrokEvent('something-failed').activity).toBe('failed')
    expect(mapGrokEvent('task-complete').normalizedType).toBe('task.completed')
    expect(mapGrokEvent('ask-user').normalizedType).toBe('permission.requested')
    expect(mapGrokEvent('subagent-start-extra').normalizedType).toBe(
      'subagent.started',
    )
    expect(mapGrokEvent('subagent').normalizedType).toBe('subagent.stopped')
    expect(mapGrokEvent('worktree-remove').normalizedType).toBe(
      'worktree.removed',
    )
    expect(mapGrokEvent('worktree').normalizedType).toBe('worktree.created')
    expect(mapGrokEvent('session-end').normalizedType).toBe('session.ended')
    expect(mapGrokEvent('session').normalizedType).toBe('session.started')
    expect(mapGrokEvent('post', 'bash').normalizedType).toBe(
      'command.completed',
    )
    expect(mapGrokEvent('pre', 'ReadFile').normalizedType).toBe('file.read')
    expect(mapGrokEvent('pre', 'ApplyPatch').normalizedType).toBe(
      'file.changed',
    )
    expect(mapGrokEvent('pre', 'Unknown').normalizedType).toBe(
      'activity.changed',
    )
  })
})
